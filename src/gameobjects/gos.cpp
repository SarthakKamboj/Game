...
const time_count_t goomba_t::TURN_WAIT_TIME = 0.2;
...
void update_goomba(goomba_t& goomba) {
	...
	for (goomba_turn_pt_t& turn_pt : goomba_turn_pts) {
		...
		if (glm::dot(diff_vec, diff_vec) <= 1.f && (goomba.last_turn_time + goomba_t::TURN_WAIT_TIME <= platformer::time_t::cur_time)) {
			goomba.last_turn_time = platformer::time_t::cur_time;
...
void update_parallax_bcks(camera_t& camera) {
	...
	float bck_width = app.window_height * static_cast<float>(bck_tex->width) / bck_tex->height;

	for (int i = 0; i < PARALLAX_BCK::NUM_BCKS; i++) {
		set_quad_width_height(parallax_bck::rec_render_handles[i], bck_width, app.window_height);
	}

	even_bck_1->position.y = ground_height + ((app.window_height-ground_height)/2);
	even_bck_2->position.y = ground_height + ((app.window_height-ground_height)/2);
	odd_bck_1->position.y = ground_height + ((app.window_height-ground_height)/2);
	odd_bck_2->position.y = ground_height + ((app.window_height-ground_height)/2);

	int cam_x = camera.pos.x;
	int cam_center_x_screen = cam_x + (app.window_width / 2);
